        ...
        VStack(spacing: 16) {
            ...
            if filteredCities.isEmpty {
                EmptyStateView(
                    ...
                    description: cities.isEmpty ? "No cities in this district yet" : "Try adjusting your filters"
                )
            } else {
                ScrollView {
    ...
    private var filteredCities: [CityPerformance] {
        ...
        if !searchText.isEmpty {
            result = result.filter {
                ...
                $0.cityHeadName.localizedCaseInsensitiveContains(searchText)
            }
        }
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 12) {
                ...
                CityMetric(
                    ...
                    label: "Revenue",
                    value: "₹\(String(format: "%.1f", Double(city.revenue)/1000000))M"
                )
            ...
            HStack {
                ...
                VStack(spacing: 8) {
                    HStack(spacing: 12) {
                        Label("\(city.powerOutages)", systemImage: "bolt.slash.fill")
                            .font(.caption)
                            .foregroundColor(.red)
                        
                        Label("\(city.pendingComplaints)", systemImage: "exclamationmark.triangle.fill")
                            .font(.caption)
                            .foregroundColor(.orange)
                    }
                }
