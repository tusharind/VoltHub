...
final class SignUpViewModel: ObservableObject {
    ...
    func validatePassword(_ password: String) -> Bool {
        ...
        guard password.range(of: uppercasePattern, options: .regularExpression) != nil else {
            passwordError = "Password must contain at least one uppercase letter"
            return false
        }
        
        let lowercasePattern = ".*[a-z]+.*"
        guard password.range(of: lowercasePattern, options: .regularExpression) != nil else {
            passwordError = "Password must contain at least one lowercase letter"
            return false
        }
        
        let digitPattern = ".*[0-9]+.*"
        guard password.range(of: digitPattern, options: .regularExpression) != nil else {
            passwordError = "Password must contain at least one number"
            return false
        }
        
        let specialCharPattern = ".*[!@#$%^&*(),.?\":{}|<>]+.*"
        guard password.range(of: specialCharPattern, options: .regularExpression) != nil else {
    ...
    func signUp() async {
        guard isFormValid else {
            state = .failure(NSError(domain: "", code: 0,
                        userInfo: [NSLocalizedDescriptionKey: "Please fill all fields correctly"]))
            return
        }
    ...
    let endpoint = Endpoint(path: "/signup", method: .post, body: body, requiresAuth: false)
